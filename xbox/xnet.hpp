...
namespace xnet{
    ...
    namespace details{
        ...
        static void prep_sendfile(io_uring_sqe* sqe, int sockfd, int filefd, int64_t offset, unsigned int bytes, unsigned int flags){
    ...
    public:
        ...
        int run() noexcept{
            ...
            int& result_scope = this->result();
            int num_submitted = this->submit();
            if(num_submitted <= 0){
                return -num_submitted;
            }
            while(true){
                unsigned int count = io_uring_peek_batch_cqe(&this->ring, cqes, cqe_size);
                for(unsigned int i = 0; i < count; i++){
                    ...
                    // }
                    if(handle){
                        result_scope = cqe->res;
                        handle.resume();
                    }
                }
                io_uring_cq_advance(&this->ring, count);
                num_submitted = io_uring_submit_and_wait(&this->ring, 1);
                if(num_submitted < 0){
                    return -num_submitted;
                }
            }
        ...
        class Accepter;
        using v4TCPAccepter = Accepter<AF_INET, SOCK_STREAM>;
        using v6TCPAccepter = Accepter<AF_INET6, SOCK_STREAM>;
        ...
        public:
            ...
            Accepter(Accepter&& other) noexcept: ctx(other.ctx), server(std::exchange(other.server, INVALID_HANDLE))
            {}
            void operator=(Accepter&& other) = delete;
            ~Accepter(){
                this->close();
            }
        private:
            class [[nodiscard]] AcceptAwaiter{
                ...
                public:
                    AcceptTimeoutAwaiter(Accepter& accepter, uint32_t s, uint32_t ns) 
                    ...
                    Accepter& sender() noexcept { return this->accepter; }
                    bool pending() const noexcept { return this->handler != nullptr; }
                    std::coroutine_handle<>& handle() { return this->handler; }
                    CancelHandle cancelhandle() { return CancelHandle(this); }
                    ...
                    details::io_result<bool> cancel() noexcept{
                        using func_type = decltype(io_uring_prep_cancel);
                        using data_type = void*;
                        return io_context::cancel<Accepter, func_type, io_uring_prep_cancel, data_type>(
            ...
            public:
                AcceptAwaiter(Accepter& accepter) noexcept :accepter(accepter), handler(nullptr)
                {}
